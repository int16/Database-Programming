...
import scores.MultiEvent.MultieventRule;
...
public class EventData
{
	...
	public void storeNewMultiEvent(MultiEvent me) throws SQLException, ClassNotFoundException
	{
		...
		stmt.executeUpdate("INSERT INTO multieventcompetition"
				+ " (comp_name, period_start, period_end) VALUES ('"+me.getName()+"', '"
				+(new java.sql.Date(me.getStart().getTime()))+"', '"+(new java.sql.Date(me.getEnd().getTime()))+"');");
		...
		PreparedStatement pstmt =  conn.prepareStatement("INSERT INTO multieventrule "
				+ "(multieventcomp_id, round_id, calc_rules,  events_included) "
				+ "VALUES (?, ?, ?, ?);");
	...
	public void storeNewEvent(Event e) throws SQLException, ClassNotFoundException
	{
		...
		int multiEventId = 0;
		if (e.getMultiEvent() != null)
			multiEventId = e.getMultiEvent().getId();

		String dateString = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.ENGLISH).format(e.getEventTime());

		stmt.executeUpdate("INSERT INTO event"
				+ " (eventname, venue, round_id, multieventcomp_id, time_shot) "
				+ "VALUES ('"+e.getName()+"', '"+e.getVenue()+"', "
				+ e.getRound().getId()+", "+multiEventId+ ", '"
				+dateString+"');");
