...
public class MultiEventEditingPN extends JPanel
{
	...
	private void addRules()
	{
		...
		if(rulesCounter >= rulesPNs.length)
		{
			JOptionPane.showMessageDialog(this,
					"Max 10 rules. Blame the person who wrote this program. ",
					"Number of Rules Exceeded",
		...
		else
		{
			...
			{
				JOptionPane.showMessageDialog(this,
						"You have to include some rounds, official or unofficial. ",
						"Try again",
	...
	private void fetchRounds(boolean official, boolean inofficial)
	{
		...
		{
			JOptionPane.showMessageDialog(this,
					"Blame the person who wrote this program. "+e.getStackTrace(),
					"Database Problems",
					JOptionPane.ERROR_MESSAGE);
		}
	...
	private class RulesAddingListener implements ActionListener
	{
		...
		{
			if(event.getSource().equals(rulesBTN))
				addRules();
			else
			{
				int which = -1;
				...
				{
					if(event.getSource().equals(removeBTN[i]))
						which = i;
				}
				removeRules(which);
			}
