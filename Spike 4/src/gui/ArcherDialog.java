...
public class ArcherDialog extends JDialog implements ActionListener 
{
	...
	public ArcherDialog(JFrame frame, boolean modal, Archer[] archers, 
			boolean includeArchived, Equipment[] equip, Discipline[] disc, Category[] cats)
	{
	...
	private void selectCategoryForArcher(Archer a)
	{
		...
		try
		{
			...
			{
				if(newCat[i].getId() == a.getCat().getId())
					archerCat = i;
	...
	private void selectEquipmentForArcher(Archer a)
	{
		...
		{
			if(equipment[i].getId() == a.getDefaultEquipment().getId())
				break;
		}
	...
	private void selectDisciplineForArcher(Archer a)
	{
		...
		{
			if(discipline[i].getId() == a.getDefaultDiscipline().getId())
				break;
		}
