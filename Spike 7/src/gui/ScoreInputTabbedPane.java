...
public class ScoreInputTabbedPane extends JPanel {
		...
		JTable table = new JTable(rtm) {
			...
			public void changeSelection(final int row, final int column, boolean toggle, boolean extend) {
				...
				if (!isCellEditable(row, column) && row + 1 < getRowCount()) {
					// here we want to make sure the row exists and has not been filled in yet - lest we overwrite the
					// next row
					if (super.getModel().getValueAt(row + 1, 0).equals("")) changeSelection(row + 1, 0, toggle, extend);
				} else {
	...
	private class TabChangeAction extends AbstractAction {
		...
		@Override
		public void actionPerformed(ActionEvent e) {
			if (tabbedPane.getSelectedIndex() + 1 < tabbedPane.getTabCount()) tabbedPane.setSelectedIndex((tabbedPane.getSelectedIndex() + 1));
		}
	}
