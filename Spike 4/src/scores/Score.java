...
public class Score
{
	...
	{
	    scores[i] = new String[round.getRange(i).getEnds()][round.getRange(i).getArrowsPerEnd()];
	}
	...
	for (int i = 0; i < rangeData.length; i++)
	{
		...
		else if(!rangeData[i][j].equals("") && !rangeData[i][j].equalsIgnoreCase("m"))
		    rangeTotals[rangeCounter] += Integer.parseInt(scores[rangeCounter][i][j]);
		if(rangeData[i][j].equals("10")) 
		    total10s++;
    ...
    {
	if(rangeCounter==round.getRangeCount())
	    dnf=true;
	//if the last entry is empty, we can assume the score is incomplete
	else if (rangeData[rangeData.length-1][rangeData[rangeData.length-1].length-2].equals(""))
	    dnf = true;
	...
	for (int i = 0; i < scores[rangeCounter].length; i++)
	{
	    ...
	    {
		if(scores[rangeCounter][i][j].equalsIgnoreCase("x"))
		    endScore+= 10;
		else if (!scores[rangeCounter][i][j].equals("") && !scores[rangeCounter][i][j].equalsIgnoreCase("m"))
		    endScore += Integer.parseInt(scores[rangeCounter][i][j]) ;
	    }
		...
		a.setEndNo(i+1);
		//stupid handbrake
		if (awardTypes[1].getDescription().indexOf("Perfect") != -1)
		    a.setAwardType(awardTypes[1]);
		a.setRange(r);
		a.setRound(round);
		...
		{
		    if(!scores[rangeCounter][i][j].equalsIgnoreCase("x") &&
			    !scores[rangeCounter][i][j].equals("10") &&
			    !scores[rangeCounter][i][j].equals("9"))
		    {
			isAllGold = false;
			break;
		    }
		...
		if(isAllGold)
		{
		    Award a = new Award(round, r);
		    a.setEnd(scores[rangeCounter][i]);
		    if (awardTypes[0].getDescription().indexOf("All") != -1)
			a.setAwardType(awardTypes[0]);
		    a.setRange(r);
		    a.setRound(round);
		    a.setEndNo(i+1);
		    awards.add(a);
